...
public class Thread implements Runnable {
    ...
    public synchronized void start() {
         ...
         *
         * 首先会判断threadStatus是否为0，如果不为0会抛出异常。然后会将当前对象添加到线程组，最后调用start0方法，
         *
         * 会抛出IllegalThreadStateException异常。其实在Thread#start()方法里面的的注释中有提到，多次调用start()方法是非法的，
         * 所以在上面的start()方法源码中一开始就是对threadStatus进行判断，不为0就会抛出IllegalThreadStateException异常。
         */
        if (threadStatus != 0)
     ...
     * Runnable   之所以有这种实现方法，是因为Thread类的run方法中会判断成员变量target是否为空，不为空就会调用target类的run方法。
     */
    @Override
    public void run() {
        //线程的默认自动销毁途径：执行结束 run() 方法就退出了
        if (target != null) {
            target.run();
        }
    }
     ...
     * 上面还提到了,如果阻塞线程调用了 interrupt0方法,那么会抛出异常,设置标志位为 false,同时该线
     * 程会退出阻塞的。
    ...
    public void interrupt() {
        ...
        synchronized (blockerLock) {
            ...
            if (b != null) {//看看是不是阻塞的线程调用(如:sleep)
                interrupt0();           // Just to set the interrupt flag
                b.interrupt(this);//如果是,抛出异常,将中断标志位改了[为false](-般我们会再 catch中再次修改)
                return;
            }
        }
        interrupt0();//如果很顺利,就可以修改到标志位了(没有进入f)
    }
