...
public class ArrayBlockingQueue<E> extends AbstractQueue<E> implements BlockingQueue<E>, java.io.Serializable {
     ...
     * are known not to be any.  Allows queue operations to update iterator state.
     */
    ...
    private void enqueue(E x) {
        ...
        items[putIndex] = x;
        // 如果下一个 位置 已经满了，那么就从头开始，没啥毛病啊
        if (++putIndex == items.length) {
            putIndex = 0;
        }
        count++;
        //当作生产者 生产元素到容器中，并且唤醒 相关的阻塞状态的 消费者 进行获取消息
        notEmpty.signal();
    }
    ...
    private E dequeue() {
        ...
        //获取当前索引的元素，自增后，假如等于长度，就从头开始 取；
        //并没有发现：一个数组中，取头元素后，之后的元素进行 复制补位；
